...
const Contact = () => {
  const [isFlipped, setIsFlipped] = useState(false); // controls form flip
  const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  // Vite env (fallback to empty string)
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // If your API returns { success: true } on success
      if (response?.data?.success) {
        setSubmitStatus('success');
        reset();
        // flip form to success side
        setIsFlipped(true);

        // flip back after a short delay (optional)
        setTimeout(() => {
          setIsFlipped(false);
          setSubmitStatus('');
        }, 4500);
      ...
      <div className="container mx-auto px-6 relative">
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div style={{ perspective: 1100 }} className="relative">
            <motion.div
              // rotateY to flip between front (0) and back (180)
            ...
            >
                    ...
                    {submitStatus === 'error' && (
                      <div className="p-4 bg-red-900 border border-red-600 text-red-100 rounded-lg flex items-start space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs">Please try again or check your network.</p>
                        </div>
                      </div>
                    )}
              ...
              {/* BACK: success card (rotated 180 so it becomes visible when wrapper rotates) */}
              <div
                style={{
                  ...faceStyle,
                  position: 'absolute',
              ...
              >
                {submitStatus === 'success' ? (
                  <div className="text-center">
                    ...
                    <p className="text-2xl font-bold text-green-200">Message Sent Successfully!</p>
                    <p className="text-sm text-green-100 mt-2">I'll get back to you within 24–48 hours.</p>
                  </div>
                ) : (
                  // In the unlikely case the back is shown while status isn't 'success'
                  <div className="text-center">
                    <p className="text-2xl font-bold text-yellow-200">Status</p>
                    <p className="text-sm text-yellow-100 mt-2">Currently processing...</p>
                  </div>
                )}
              </div>
            </motion.div>
